...
static int	extract_mobius(t_ray *tr, double tt)
{
	t_vec3	p;
	double	t;
	double	s;
	...
	p = find_intersection_point(tr, &tt);
	t = atan2(p.y, p.x);
	prm = t / 2.;
	s = p.z / sin(prm);
	p = v3_dif(p, v3((1.5 + s * cos(prm)) * cos(t),
		(1.5 + s * cos(prm)) * sin(t), s * sin(prm)));
	if ((v3_dot(p, p) < 0.0000000001))
		if (s >= -0.5 && s <= 0.5)
			return (1);
	return (0);
}
